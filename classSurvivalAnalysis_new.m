classdef classSurvivalAnalysis
    ...
    methods
        function SAobj = classSurvivalAnalysis(mSurvivalTime,mFlgCensor,mFlgCompareWithVariations)
        ...
        function SAobj = set.mSurvivalTime(SAobj,survivaltime)
            if ~isa(survivaltime,'cell')
                error('The mSurvivalTime in classSurvivalAnalysis should be a cell array');
            end
%             if all(cellfun('isempty',survivaltime))
%                 error('The mSurvivalTime in classSurvivalAnalysis is assigned by empty cell(s)');
%             end
...
%                 error('The mFlgCensor in classSurvivalAnalysis is assigned by empty cell(s)');
%             end
...
% sort data by survival time, and generate histogram table
    ...
    if any(mFlgCensor)
        CensorTimeTable=tabulate(mSurvivalTimeTable(mFlgCensor)); CensorTimeTable(CensorTimeTable(:,2)==0,:)=[]; % CensorTimeTable - table of censors (the censor time)
    end
    mSurvivalTimeTable=tabulate(mSurvivalTimeTable); mSurvivalTimeTable(mSurvivalTimeTable(:,2)==0,:)=[]; % survival table. for interger survival times not observed time added by tabulate should be removed
